...
"""
    ...
    if growing_season == True:
        ...
        if IrrMngt_IrrMethod == 0:
            ...
            next_day_SM = 0
            current_SM = SMs[NewCond_TimeStepCounter]
            try:
                next_day_SM = FSMs[NewCond_TimeStepCounter]
            except Exception as e:
                next_day_SM = current_SM

            #print(f'Current SM: {current_SM} | Predicted SM: {next_day_SM}')
            ...
            if (Dr > 1 - IrrMngt_SMT[index] / 100):                
                ...
                AdjNextDaySoilMoinsture = 0                
                IrrReq = (IrrReq * EffAdj) - AdjNextDaySoilMoinsture
                # Limit irrigation to maximum depth                
                if next_day_SM > current_SM and TYPE==1:        
                    #print('Discounting SM')                                                                
                    Irr = min(IrrMngt_MaxIrr, IrrReq) - ((next_day_SM-current_SM))                                           
                else:
                    #print('No Discounting SM')                                                                
